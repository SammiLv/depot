...
export default function TalentPageContent({
  ...
  return (
    <div className="flex h-full min-h-0 flex-col bg-[#F5F7F9]">
      ...
      {section === "decision" && operationWorkspace.decision && <TalentDecisionWorkspace data={operationWorkspace.decision} />}
      {section === "history" && operationWorkspace.history && <TalentHistoryWorkspace data={operationWorkspace.history} employeeProfiles={operationWorkspace.employeeProfiles} initialCategory={historyInitialCategory} />}
      {section === "config" && operationWorkspace.career && operationWorkspace.competency && <ConfigWorkbench data={reviewWorkspace} career={operationWorkspace.career} competency={operationWorkspace.competency} decisionRules={operationWorkspace.decisionRules} activeConfig={activeConfig} onSelect={setActiveConfig} onBack={() => setActiveConfig(null)} onNotice={showNotice} />}
        </Card>
        </div>
      )}
      </div>
...
function reviewStatusLabel(status: string) { return ({ IN_PROGRESS: "进行中", CALIBRATING: "校准中", CONFIRMED: "已完成", ARCHIVED: "已归档" } as Record<string, string>)[status] ?? status; }
