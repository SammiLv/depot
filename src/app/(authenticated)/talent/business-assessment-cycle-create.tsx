...
function departmentTeams(data: AssessmentWorkspaceData, departmentOrgNodeId: string | null) {
  const teams = data.teams.filter((team) => team.parentId === departmentOrgNodeId);
  if (data.managesFullDepartmentTeams) return teams;
  const allowed = new Set(data.manageableTeamIds ?? []);
  ...
  return <form action={formAction} className="space-y-4">
    ...
    <Card>
      <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
        <div><h3 className="font-semibold">考核科目与小组及格标准</h3><p className="mt-1 text-xs leading-5 text-muted-foreground">{data.managesFullDepartmentTeams ? "每个科目先确定评分方式，再分别设置本部门各小组的及格线，所有小组都必须配置；个人规则仅作为同科目下的特殊覆盖。" : "按你的组织权限，仅需配置你可管理的小组及格标准；个人规则仅作为同科目下的特殊覆盖。"}</p></div>
