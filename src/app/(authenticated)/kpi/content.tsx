...
export function KpiContent({ data, assessmentData, incidentData, selectedYear, selectedQuarter }: Props) {
  ...
  const scopedStages = useMemo(() => ([
    { label: "初始化", count: rows.filter((row) => row.completedProgressStages.init).length },
    { label: "自评", count: rows.filter((row) => row.completedProgressStages.selfReview).length },
    { label: "组长评", count: rows.filter((row) => row.completedProgressStages.leader).length },
    { label: "主管评", count: rows.filter((row) => row.completedProgressStages.manager).length },
    { label: "终审", count: rows.filter((row) => row.completedProgressStages.final).length },
  ]), [rows]);
