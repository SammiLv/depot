...
async function manager() { const user = await requireCurrentUser(); const permission = await resolvePermissionCoverage(user, orgPermissionModuleKeys.kpi, kpiAbilityKeys.manageWorkIncident); if (!permission.hasPermission) throw new Error("没有工作事故管理权限"); return user; }
async function assertDepartment(user: Awaited<ReturnType<typeof requireCurrentUser>>, departmentOrgNodeId: string) { const ids = await resolveAuthorizedOrgNodeIds(user, orgPermissionModuleKeys.kpi, kpiAbilityKeys.manageWorkIncident); if (ids !== null && !ids.includes(departmentOrgNodeId)) throw new Error("不能管理该部门的工作事故"); }
...
export async function createWorkIncident(formData: FormData) {
  ...
  const targetUserId = required(formData, "userId"); const target = await prisma.user.findFirst({ where: { id: targetUserId, isActive: true, deletedAt: null }, select: { orgNodeId: true } }); if (!target?.orgNodeId) throw new Error("责任人不存在"); const allowed = await resolveAuthorizedOrgNodeIds(user, orgPermissionModuleKeys.kpi, kpiAbilityKeys.manageWorkIncident); if (allowed !== null && !allowed.includes(target.orgNodeId)) throw new Error("不能录入该员工的工作事故");
