...
2. 一个终端一个分支。
3. 默认只开 3 个终端。
4. 按本轮要改的菜单动态分工，不按大模块永久分组。
5. 第 3 个终端负责平台收口；涉及 DB 时优先负责 DB 协调。

一句话：**两个菜单终端 + 一个平台终端**；如有 DB，再由平台终端优先处理 DB，必要时才临时开第 4 个终端。
...
- 年度指标：`../depot-annual-goals` / `feat/menu-annual-goals-<topic>`
- KPI 管理：`../depot-kpi` / `feat/menu-kpi-<topic>`
- 季度工作：`../depot-quarterly-work` / `feat/menu-quarterly-work-<topic>`
- 通知中心：`../depot-notifications` / `feat/menu-notifications-<topic>`
- 平台终端：`../depot-platform` / `feat/platform-<topic>`
- DB 终端：`../depot-db` / `feat/db-<topic>`
...
1. 先确定本轮要改哪 2 个菜单。
2. 再确定是否涉及 DB。
3. 创建 2 个菜单 worktree + 1 个 platform worktree。
