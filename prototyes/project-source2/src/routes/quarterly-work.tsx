  ...
  return (
    <AppShell>
      <div className="mb-6 flex items-end justify-between flex-wrap gap-3">
        <div>
          <h2 className="text-2xl font-semibold tracking-tight text-foreground">2025 Q4 季度工作</h2>
          <p className="text-sm text-muted-foreground mt-1">按小组规划季度工作 · 月度拆解 · 每周更新进展，延期自动预警；上线后跟踪需求价值</p>
        </div>
        <Button><Plus className="w-4 h-4" />新增季度工作</Button>
      </div>

      <div className="mb-4 rounded-xl bg-card/70 backdrop-blur-xl border border-border p-5 shadow-sm">
        <div className="inline-flex p-1 rounded-lg bg-muted/60 backdrop-blur-md border border-border/60">
          {[
            { k: "board", label: "工作看板" },
            ...
            <button
              ...
              className={`px-4 py-1.5 rounded-md text-sm transition ${
                tab === t.k ? "bg-card/90 text-foreground shadow-sm font-medium" : "text-muted-foreground hover:text-foreground"
              }`}
            >
              {t.label}
            </button>
        ...
        {tab === "board" ? (
          <div className="mt-4 flex items-center gap-2 flex-wrap">
            {["全部", "采购组", "B端组", "C端组", "设计组"].map((t, i) => (
              <button key={t} className={`px-3 py-1.5 rounded-lg text-sm transition ${i === 0 ? "bg-primary text-primary-foreground" : "bg-card/80 border border-border hover:bg-muted"}`}>
                {t}
              </button>
            ))}
      ...
      {tab === "board" ? (
        <>
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-4">
            {columns.map((c) => (
              <div key={c.key} className="bg-card/70 backdrop-blur-xl border border-border rounded-xl p-3 min-h-[400px] shadow-sm">
                ...
                <div className="space-y-2">
                  {c.items.map((it, i) => (
                    <div key={i} className="bg-background/80 backdrop-blur-md rounded-lg p-3 border border-border hover:border-primary/40 transition cursor-pointer shadow-sm">
